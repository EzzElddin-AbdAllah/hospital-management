    ...
    <div className={`w-full space-y-6 ${showCurrent ? "mb-20 mt-8 px-4" : ""}`}>
      {filteredAppointments?.length ? (
        filteredAppointments.map((appointment) => (
          <div
            dir="ltr"
            key={appointment._id}
            className={` ${showCurrent ? "" : "flex-row-reverse"} flex min-h-[200px] items-center
              justify-between rounded-3xl border bg-white px-2 shadow-lg sm:px-4
              lg:min-h-[250px] lg:py-4`}
          >
            {showCurrent && (
              <Button
                className="mb-9 place-self-end text-sm text-[#F10005] hover:text-red-700 lg:ml-9"
                variant="transparent"
            ...
            <Group className="h-fit flex-nowrap justify-end pr-3">
              <div className="flex flex-col gap-1 text-right">
                <Text className="text-lg font-bold lg:text-xl">
                  {appointment.doctor.clinic}
                </Text>
                <Text className="text-md text-color-accent-dark lg:text-lg">
                  {appointment.doctor.name}
                </Text>
                <Text className="text-md text-color-accent-medium lg:text-lg">
              ...
              <Avatar
                ...
                className={`object-contain sm:h-[150px] sm:w-[150px] ${
                  showCurrent ? "" : "hidden md:block md:scale-90" }`}
              />
            </Group>
          </div>
        ))
      ...
      >
        ...
        <Group mt="md" justify="end">
          ...
          <Button color="rgb(var(--color-error))" onClick={handleDelete}>
            حذف
          </Button>
        </Group>
      </Modal>
